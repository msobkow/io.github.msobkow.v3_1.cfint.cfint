	...
	implements ICFIntSecClusGrpIncEditObj
{
	...
	public ICFSecSecClusGrpObj getRequiredContainerGroup( boolean forceRead ) {
		if( forceRead || ( requiredContainerGroup == null ) ) {
			...
			if( ! anyMissing ) {
				ICFSecSecClusGrpObj obj = ((ICFIntSchemaObj)getOrigAsSecClusGrpInc().getSchema()).getSecClusGrpTableObj().readSecClusGrpByIdIdx( getPKey().getRequiredSecClusGrpId() );
	...
	public ICFSecSecSysGrpObj getRequiredParentSubGroup( boolean forceRead ) {
		if( forceRead || ( requiredParentSubGroup == null ) ) {
			boolean anyMissing = false;
			if( ! anyMissing ) {
				ICFSecSecSysGrpObj obj = ((ICFIntSchemaObj)getOrigAsSecClusGrpInc().getSchema()).getSecSysGrpTableObj().readSecSysGrpByUNameIdx( getPKey().getRequiredInclName() );
