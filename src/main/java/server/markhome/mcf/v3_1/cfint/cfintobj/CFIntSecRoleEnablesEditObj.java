// Description: Java 25 edit object instance implementation for CFInt SecRoleEnables.
...
public class CFIntSecRoleEnablesEditObj
	implements ICFIntSecRoleEnablesEditObj
{
	...
	public CFIntSecRoleEnablesEditObj( ICFSecSecRoleEnablesObj argOrig ) {
		orig = argOrig;
		getRec();
	...
	public ICFSecSecRoleEnablesObj realise() {
		...
		ICFSecSecRoleEnablesObj retobj = getSchema().getSecRoleEnablesTableObj().realiseSecRoleEnables( (ICFIntSecRoleEnablesObj)this );
		return( retobj );
	}
	...
	public ICFSecSecRoleEnablesObj create() {
		copyRecToOrig();
		ICFSecSecRoleEnablesObj retobj = ((ICFIntSchemaObj)getOrigAsSecRoleEnables().getSchema()).getSecRoleEnablesTableObj().createSecRoleEnables( getOrigAsSecRoleEnables() );
	...
	public ICFSecSecRoleObj getRequiredContainerRole( boolean forceRead ) {
		if( forceRead || ( requiredContainerRole == null ) ) {
			...
			if( ! anyMissing ) {
				ICFSecSecRoleObj obj = ((ICFIntSchemaObj)getOrigAsSecRoleEnables().getSchema()).getSecRoleTableObj().readSecRoleByIdIdx( getPKey().getRequiredSecRoleId() );
	...
	public ICFSecSecSysGrpObj getRequiredParentEnableGroup( boolean forceRead ) {
		if( forceRead || ( requiredParentEnableGroup == null ) ) {
			boolean anyMissing = false;
			if( ! anyMissing ) {
				ICFSecSecSysGrpObj obj = ((ICFIntSchemaObj)getOrigAsSecRoleEnables().getSchema()).getSecSysGrpTableObj().readSecSysGrpByUNameIdx( getPKey().getRequiredEnableName() );
